pipeline {
    ...
    environment {
        ...
        CODECOV_TOKEN = credentials('codecov-addon-oam-kubernetes-remote')
    }
            ...
            steps {
                script {
                    if (env.CHANGE_ID != null) {
                        def json = sh (script: "curl -s https://api.github.com/repos/crossplane/addon-oam-kubernetes-remote/pulls/${env.CHANGE_ID}", returnStdout: true).trim()
